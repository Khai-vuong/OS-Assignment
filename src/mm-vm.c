...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  else
  {
   ...
   struct vm_area_struct* curvma = get_vma_by_num(caller->mm, vmaid);
...
int __free(struct pcb_t *caller, int vmaid, int rgid)
{
  ...
  // Lấy khỏi symbol table rồi chỗ đó điền giá trị gì vào? NULL? delete? start = end?
...
int validate_overlap_vm_area(struct pcb_t *caller, int vmaid, int vmastart, int vmaend)
{
  ...
  if (curvma == NULL) {return -1;}
  if (vmastart >= vmaend) {
    return -1;
  }

  return OVERLAP(curvma->vm_start, vmastart, curvma->vm_end, vmaend);
...
int inc_vma_limit(struct pcb_t *caller, int vmaid, int inc_sz)
{
  ...
  int inc_amt = PAGING_PAGE_ALIGNSZ(inc_sz);  //amount of page required
